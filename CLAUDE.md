...
- **Callout / stat strip variants** live in `calloutExtension.ts` (`CALLOUT_VARIANTS`, including **purple** / Arcane); campaign styles in `globals.css` use **semantic hues** (grey = slate neutral, yellow = gold `#d4af37`, blue = cool blue; stat strips mirror callouts — not a single gold wash on all variants).
...
- **Rich text extensions:** `TextStyle` + `Color` + `TextAlign` on `paragraph`, `heading`, `blockquote`, `tableCell`, `tableHeader`; inline `storyTag` (variant pill; presets use **colour** display names — crimson, gold, blue, purple, orange, rose, gray) and block `statStrip` (2–6 kicker/value pairs; gutter **+** / grip for insert, drag, delete, colour, column count — gutter follows the **innermost** block, but **callout** / **toggleCard** / **statStrip** / **blockquote** / **pageEmbed** / **databaseEmbed** use one gutter pinned to that container (inset inside the chrome) so inner content doesn’t reset it; **nested** gutter blocks (e.g. **statStrip** inside **toggleCard**) get their own gutter when the cursor/selection is at that node or the gap beside it; other blocks use the margin gutter). **Tables:** `@tiptap/extension-table` (+ row/cell/header) with `resizable: true` and custom `View: LeafTableView` (`LeafTableView.ts`) for **+** append row/column chrome; slash **Table** to insert; additional slash commands with `requiresTable` filtered in `LeafEditor` `updateSlashMenu`; GFM pipe tables round-trip via `markdown-it` + `turndown-plugin-gfm` (`tables` rule). Shared presets/swatches: `frontend/src/lib/editorRichText.ts`. No static formatting bar — use slash `/` and the gutter **+** menu for blocks/marks; alignment + colours use TipTap `BubbleMenu` (`EditorSelectionBubble.tsx`) when text is selected. Grip **drag-to-columns** (left/right block edges) and **reorder** (center drop) resolve the **innermost** content block (`resolveInnermostContentBlockRange` in `LeafEditor`), including inside callouts, toggle bodies, and columns—not only top-level siblings. Slash **New database** vs **Embed database** (`database` / `database_embed`): embed-existing opens a picker (`DatabaseEmbedPickPanel`) filtered by the slash query; wire new slash actions in `SlashCommands.tsx` + `LeafEditor` `applyAction`.
...
- **Trash.** Deleted pages (`leaves.deleted_at`) and databases (`databases.deleted_at`) are retained for `TRASH_RETENTION_DAYS` (default 7), then purged by `TrashOperations` on API startup and on `GET /trash`. Restore: `POST /leaves/{id}/restore`, `POST /databases/{id}/restore`; Settings UI lists Trash.
...
└── frontend/                   # Next.js 15 App Router
    ...
    └── src/
        ...
        │   └── StatusBar.tsx # footer: save/sync status; click sync when enabled
        └── lib/
