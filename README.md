...
- **Stable editor core:** custom TipTap-based editor with slash commands and block menu (no static toolbar), selection bubble for text colour and alignment, campaign-style story tags and stat strips (2–6 cards via gutter menu), page embeds, inline database embeds, Markdown import/export, and rich/Markdown mode switching.
...
- **Editor:** TipTap rich text with slash `/` and gutter **+** / grip menu for structure and marks (no top toolbar)—the **+** insert list is the same grouped UI as slash; both panels flip upward when the anchor is in the lower third of the viewport. Atom blocks such as **stat strip** use the same gutter for insert, drag, **delete**, colour, and column count (2–6 cards), including when nested (e.g. inside a **toggle card**—select the strip or move over it so the gutter targets that block). **Selection bubble** for alignment and text colours when text is highlighted, inline **story flags** (colour-named variant pills — crimson, gold, blue, purple, orange, rose, gray; type `/flag` or a colour name in slash menu), **callout** blocks (neutral, info, arcane/purple, caution, success, danger, flavor; campaign theme keeps semantic greys/blues/yellows per variant), **stat strip** blocks (2–6 kicker/value cards, same variant palette), **markdown tables** (GFM-style; resizable columns in rich mode; **+** beside the grid adds a column, **+** under the grid adds a row; **Tab** / **Shift+Tab** move between cells; type `/` inside a table for **Table** slash group: add/remove rows and columns, header row toggle, delete table), page/database embeds, resizable column layouts (2–5), drag-to-create columns, **Toggle Cards** (collapsible blocks; eyebrow/title/subtitle use a **filtered slash menu** for text style and flags—full block slash remains in the card body), optional Markdown source mode, and Markdown import/export. **Page outline** in the right sidebar jumps the cursor to headings when clicked.
...
- **Databases:** Collections of pages as structured databases. Each entry is a real page. Views: Table, Board, Gallery, List. Schema-driven columns (text, number, date, tags, select). **Table view:** drag the right edge of the Name or property headers to resize columns; widths persist in the database schema (`name_column_width`, per-property `width`). Edit column label/type from the table header (✎ or double-click the header); **delete** a property from the same “Edit property” dialog (removes the column and its values from every row). Embedded databases use the same editor as the full-page view. Deleting a database (sidebar or embed) moves it to **Trash** (data kept until purged). Gallery cards show each row’s **cover image** when the linked page has `properties.headerBanner`. "Name" always links to the entry page. Inline cell editing and add-column flows are shared between standalone and embedded databases.
- **Trash:** Deleted **pages** and **databases** go to Trash for **7 days** (configurable via `TRASH_RETENTION_DAYS` on the API), then are **permanently deleted**. **Settings → Trash** shows a compact count by default; expand **Show deleted items** to restore, **Delete permanently** one item, or **Empty Trash now**. Opening Settings (and API startup) runs expiry cleanup.
